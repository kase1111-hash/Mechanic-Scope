...
│   ├── Performance/    # Asset optimizer, performance monitor
...
└── StandInModel/       # Generates the stand-in engine .glb files (pure Python, no dependencies)

.github/workflows/tests.yml  # CI: runs ./run_tests.sh; checks the stand-in GLBs match their generator
```
...
| Engine ID | Procedures | Notes |
|-----------|------------|-------|
| `gm_ls_gen4` | `oil_change`, `replace_alternator` | |
| `toyota_2gr_fe` | `oil_change`, `replace_spark_plugs` | **Unverified drafts**: torque values and capacities not yet checked against a service manual |

Part IDs are global across engines. Parts whose specs differ by engine get an engine suffix
(`oil_pan_toyota_2gr`, `alternator_gm_ls`); genuinely shared parts (`battery_negative`) list every
engine in their `engines` array. `ShippedDataValidationTests` runs once per folder in
`StreamingAssets/Engines`, so a new engine is validated without test changes.

## Stand-in Models

Each engine's `.glb` is generated by `python3 Tools/StandInModel/generate_standin_glb.py` from the
layouts in `STANDIN_ENGINES`. After changing a layout or the node names in an `engine.json`, rerun it
and commit the new `.glb`. CI fails if they are out of sync. When a real model replaces a stand-in,
remove that engine from `STANDIN_ENGINES`.
