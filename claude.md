...
└── StandInModel/       # Generates the stand-in gm_ls_gen4.glb (pure Python, no dependencies)

.github/workflows/tests.yml  # CI: runs ./run_tests.sh; checks the stand-in GLB matches its generator
```
...
| 3D model loading | Real, unexercised | Loads GLB via glTFast. A generated **stand-in** `gm_ls_gen4.glb` ships (boxes named to match `engine.json`); a real model is still needed |
...
`Assets/StreamingAssets/Engines/gm_ls_gen4/gm_ls_gen4.glb` is generated by
`python3 Tools/StandInModel/generate_standin_glb.py`. After changing the generator or the node
names in `engine.json`, rerun it and commit the new `.glb`. CI fails if the two are out of sync.
