...
> **Note:** The bundled `gm_ls_gen4.glb` is a crude **stand-in** — blocky shapes whose nodes are named to match `engine.json`, so the whole flow can be exercised. It is not a real engine and must not be used to locate parts. Replace it with a real model (see [Engine Models](#engine-models)). The app has not yet been run in Unity or on a device; see [Development Status](#development-status).
...
│   └── StandInModel/          # Generates the stand-in gm_ls_gen4.glb
├── Packages/
│   └── manifest.json          # Unity package dependencies
├── .github/workflows/         # CI: headless tests + stand-in model check
...
GitHub Actions runs it on every pull request and every push to `main`
...
- **The logic is correct.** Procedure dependency resolution and step sequencing, JSON parsing, the
  ...
  resolvable, and every node `engine.json` maps present in the shipped `.glb`).
...
The repo ships a **stand-in** `gm_ls_gen4.glb`, generated by
[`Tools/StandInModel/generate_standin_glb.py`](Tools/StandInModel/generate_standin_glb.py). It is
boxes and cylinders with the right node names and rough positions, enough to test loading, tapping
and highlighting. To swap in a real model, overwrite the `.glb` and make sure its node names match
`partMappings[].nodeNameInModel` in `engine.json` (the test suite checks this). Also delete the
`standin-model` job from `.github/workflows/tests.yml`, because it would otherwise regenerate the
stand-in and fail on the difference.
...
Everything below is implemented and compiles; **none of it has been run in the Unity Editor or on a
device yet.** Treat "Implemented" as "code exists and its logic is tested headlessly", not "verified
in AR".
...
- [x] GLB/glTF model loading via glTFast, with MeshColliders generated for tapping
- [x] Part tap-to-identify via raycasting
- [x] Part highlighting (URP Fresnel + outline shader, wired to the current step)
- [x] Progress persistence and resume (JSON store)
...
- [ ] **Real engine model** — the bundled `.glb` is a stand-in; see [Engine Models](#engine-models).
- [ ] **More content** — one engine (GM LS Gen IV) with two procedures.
