...
│   └── StandInModel/          # Generates the stand-in engine models (.glb)
├── Packages/
...
Each bundled engine ships a **stand-in** `.glb`, generated by
[`Tools/StandInModel/generate_standin_glb.py`](Tools/StandInModel/generate_standin_glb.py). They are
boxes and cylinders with the right node names and rough positions, enough to test loading, tapping
and highlighting. To swap in a real model, overwrite the engine's `.glb` and make sure its node
names match `partMappings[].nodeNameInModel` in its `engine.json` (the test suite checks this).
Then remove that engine from `STANDIN_ENGINES` in the generator. Otherwise the generator, and the
CI job that runs it, will overwrite your model with the stand-in.
